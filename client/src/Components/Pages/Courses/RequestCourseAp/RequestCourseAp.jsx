...
export const RequestCourseAp = () => {
  ...
  const handleSendRequest = async () => {
    try {
      ...
      const pdfBlob = await worker.output ? await worker.output('blob') : await worker.outputPdf('blob');
      ...
      // 1. Enviar solicitud
      const response = await axiosInstance.post('/api/actas/solicitud-cursoAp', formData, {
      ...
      await axiosInstance.post('/api/notifications/solicitud-curso', {
        ...
        mensaje: `El aprendiz ${aprendiz?.nombres || ''} ${aprendiz?.apellidos || ''} ha solicitado un curso complementario.`,
        archivo: archivoPDF,
      });
      ...
      <Main>
        <div className="course-request-container">
          ...
          <p className="description">
            Este documento permite a un aprendiz formalizar la solicitud de un curso ante el SENA. <br />
            Escribe el nombre del curso y las fechas de inicio y fin del curso.
          </p>
          ...
          <div className="request-card">
            ...
            <div className="letter-content apa-style" ref={pdfRef}>
              <p>
                <b>
                  {(aprendiz?.nombres || '[Nombre aprendiz]')} {(aprendiz?.apellidos || '')}
                </b>
                <br />
                Documento: {aprendiz?.documento || '[Documento]'} <br />
                Teléfono: {aprendiz?.celular || '[Teléfono]'} <br />
                ...
                Armenia - Quindío<br />
                <br />
                Asunto: Solicitud de Curso de formación Complementaria<br />
                <br />
                Respetados señores:<br />
                <br />
                Yo, <b>{aprendiz?.nombres} {aprendiz?.apellidos}</b>, aprendiz interesado en continuar mi proceso de
                formación, me permito solicitar de manera formal la apertura del curso complementario en&nbsp;
                {isEditing ? (
                  <input
                    type="text"
                ...
                {isEditing ? (
                  ...
                  <b>
                    {`${formatDate(exportValues.fechaInicio) || '[Fecha inicio]'} a ${
                      formatDate(exportValues.fechaFin) || '[Fecha fin]'
                    }`}
                  </b>
                ) : (
                  <b>
                    {`${formatDate(fechaInicio) || '[Fecha inicio]'} a ${
                      formatDate(fechaFin) || '[Fecha fin]'
                    }`}
                  </b>
                )}
                .<br />
                <br />
                Agradezco su atención y la oportunidad de fortalecer mis competencias a través de este programa.
                <br />
                <br />
                Atentamente,<br />
                <br />
          ...
          <div className="botones-solicitud">
            {isEditing ? (
              <button className="submit-button" onClick={handleSave} disabled={!!dateError}>
                Guardar
              </button>
            ) : (
            ...
            <button className="submit-button" onClick={handleSendRequest}>
              Enviar Solicitud
            </button>
          </div>
        </div>
