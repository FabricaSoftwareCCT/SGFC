...
async function ensureCursoAccesible({ cursoInput, user }) {
	ensureInitialized();
	...
	if (isPrivilegedAccount(accountTypeRaw)) {
		if (
			...
			Number(cursoData.instructor_ID) !== normalizeUserId(user)
		) {
			throw createHttpError(403, "No eres el instructor asignado a este curso.");
		}
		return cursoData;
	}
...
async function ensureActividadEstadoActual(actividadInput) {
	...
	if (shouldClose) {
		...
		return refreshed?.get ? refreshed.get({ plain: true }) : refreshed;
	}
...
class ActividadService {
	...
	static async createActividad(cursoId, payload, user) {
		try {
			...
			const actividad = await ActividadRepository.createActividad({
				...
				porcentaje_aporte: porcentajeAporte ?? null,
				creado_por: userId,
			});
			...
			return ensureActividadEstadoActual(actividadCompleta);
		} catch (error) {
			if (error?.status) {
				throw error;
			}
	...
	static async listarActividadesDelCurso(cursoId, user) {
		try {
			...
			await ensureCursoAccesible({ cursoInput: cursoId, user });
			const actividades =
	...
	static async actualizarActividad(actividadId, payload, user) {
		try {
			...
			if (payload?.descripcion !== undefined) {
				updates.descripcion = (payload.descripcion || "").trim();
			}

			let nextDeadline = actividad.fecha_limite
			...
			if (payload?.porcentajeAporte !== undefined) {
				const aporte = Number(payload.porcentajeAporte);
			...
			if (payload?.estado !== undefined) {
				const estadoLower = String(payload.estado).toLowerCase();
				if (!["activa", "cerrada"].includes(estadoLower)) {
					throw createHttpError(400, "El estado proporcionado no es válido.");
				}
				updates.estado = estadoLower;
			...
			return ensureActividadEstadoActual(updated);
		} catch (error) {
			if (error?.status) {
				throw error;
			}
	...
	static async registrarEntrega(actividadId, { comentario }, file, user) {
		try {
			...
			if (existingEntrega) {
				...
				return updatedEntrega.get ? updatedEntrega.get({ plain: true }) : updatedEntrega;
			}
			...
			return entregaCompleta.get ? entregaCompleta.get({ plain: true }) : entregaCompleta;
		} catch (error) {
			if (error?.status) {
				throw error;
			}
	...
	static async revisarEntrega(entregaId, payload, file, user) {
		try {
			...
			updates.retro_by = normalizeUserId(user);
			...
			return entregaActualizada.get ? entregaActualizada.get({ plain: true }) : entregaActualizada;
		} catch (error) {
			if (error?.status) {
				throw error;
			}
